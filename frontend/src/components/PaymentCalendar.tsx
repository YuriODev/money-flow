...
export function PaymentCalendar({ onDateClick }: PaymentCalendarProps) {
  ...
  const { data: events = [], isLoading: isLoadingEvents, dataUpdatedAt } = useQuery({
  ...
  console.log(`[Calendar] Query state: events=${events.length}, dataUpdatedAt=${dataUpdatedAt}, isLoading=${isLoadingEvents}`);
  ...
  const handleTogglePayment = (event: CalendarEvent) => {
    if (event.is_paid) {
      ...
      // Mark as paid - show warning if no card but still allow marking
      if (!event.card_id) {
        setNoCardWarning(event.id);
        setTimeout(() => setNoCardWarning(null), 3000);
        // Still proceed to mark as paid even without a card
      }
        ...
        {selectedDate && selectedDayEvents.length > 0 && (
          ...
          >
            <motion.div
              ...
              className="bg-white dark:bg-gray-900 rounded-2xl shadow-xl max-w-sm w-full overflow-hidden"
            ...
            >
              ...
              <div className="px-5 pt-5 pb-4 border-b border-gray-100 dark:border-gray-800">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <h3 id="day-details-title" className="text-lg font-semibold text-gray-900 dark:text-gray-100 mt-0.5">
              ...
              {/* Payment List */}
              <div className="px-5 py-4 max-h-[50vh] overflow-y-auto" role="list" aria-label="Payments for this day">
                <div className="space-y-3">
                  {selectedDayEvents.map((event, index) => {
                    const serviceInfo = findService(event.name);
                    const brandColor = serviceInfo?.color || "#6366F1";
                    const iconUrl = serviceInfo?.iconUrl || (serviceInfo?.icon ? getIconUrl(serviceInfo.icon, serviceInfo.color) : null);
                    ...
                    return (
                      ...
                      >
                        <div className={cn(
                          ...
                          <div
                            className={cn(
                              ...
                              iconUrl ? "bg-white dark:bg-white/90" : ""
                            )}
                            style={!iconUrl ? { backgroundColor: brandColor } : undefined}
                          >
                            {iconUrl ? (
                              <img
                                src={iconUrl}
                          ...
                          <div className="flex-1 min-w-0">
                            <p className={cn(
                              ...
                              isPaid ? "text-gray-500 dark:text-gray-400 line-through" : "text-gray-900 dark:text-gray-100"
                            )}>{event.name}</p>
                            ...
                            className={cn(
                              ...
                              isPaid
                                ...
                                : isLoading
                                  ...
                                  : "bg-white dark:bg-gray-800 border-gray-200 dark:border-gray-700 hover:border-emerald-400 hover:bg-emerald-50 dark:hover:bg-emerald-900/30"
                            )}
                            aria-label={isPaid ? `Unmark ${event.name} as paid` : `Mark ${event.name} as paid`}
                            aria-pressed={isPaid}
                          >
                            {isLoading ? (
                        ...
                        {/* No Card Warning - shows when payment is marked without a card */}
                        <AnimatePresence>
                          {showNoCardWarning && (
                            <motion.div
                              ...
                              className="mt-2 px-3 py-2 bg-amber-50 dark:bg-amber-900/30 border border-amber-200 dark:border-amber-700 rounded-lg"
                            >
                              <p className="text-xs text-amber-700 dark:text-amber-400 flex items-center gap-1.5">
                                <CreditCard className="w-3.5 h-3.5" />
                                No card assigned - consider linking a payment card
                              </p>
                            </motion.div>
                          )}
                        </AnimatePresence>
              ...
              {/* Footer with Total */}
              <div className="px-5 py-4 bg-gray-50 dark:bg-gray-800 border-t border-gray-100 dark:border-gray-800">
                <div className="flex items-center justify-between">
